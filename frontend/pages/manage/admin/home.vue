    ...
    <v-col cols="12" md="8">
            ...
            <v-card>
                <v-container>
                    ...
                    <v-data-table
                        ...
                        show-select
                        class="elevation-1"
                        :loading="this.all_loading_state"
                        loading-text="불러오는 중입니다..."
                        no-data-text="고객이 존재하지 않습니다"
                    >
                    </v-data-table>
        ...
        >
            ...
            <v-btn type="text"
                ...
                class="customer-btn"
                :disabled="this.selected.length == 1? false:true"
                @click.prevent="delete_dialog = true"
            >고객 삭제하기</v-btn>
            <v-dialog
                    ...
                    <v-container>
                        <div>
                            ...
                            <v-btn type="text"
                                color="rgb(0, 212, 150)"
                                class="submit-btn-1"
                                @click.prevent="onDeleteCustomer"
                            >삭제하기</v-btn>
                            <v-btn type="text"
                                color="#eee"
                                class="submit-btn-2"
                                @click.prevent="delete_dialog = false"
                            >취소하기</v-btn>
                        </div>
                    </v-container>
                </v-card>
...
export default {
    ...
    methods: {
        ...
        onDeleteCustomer(){
            this.$store.dispatch('customers/deleteCustomer', {
                    ...this.selected
            }).then((data) => {
                console.log('succeed');
            }).catch((err) => {
        ...
        onClickCustomer(){
            this.$router.push({
                path: '/manage/admin/customer/'+this.selected[0].id,
            })
        }
    },
